...
def call_scrape_function():
    ...
    for key, value in DOCUMENT_MAP.items():
        print(f"\nProcessing {key} document...")

        scrape_data(value['link'], temp_agreement)

    # Step-I: Extract clauses from the agreement
        # classco = data_extraction.Clause_extraction(temp_agreement)

    # Step-II: Update respective json file with new clauses (dpa.json)
        # with open(value['json_file'], "w", encoding='utf-8') as f:
        #     json.dump(classco, f, indent=2, ensure_ascii=False)

        print(f"{key} extraction completed and saved to {value['json_file']}")
